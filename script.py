    ...
    for entry in riplist:
        ...
        if TK in RTable:	# we have an existing entry for this destination
            ...
            if newcost < currentcost:		# lower-cost route
                ...
                call_list = ['ip', 'route', 'change', f"{ipaddr}/{slash(aton(netmask))}", 'via', update_sender, 'metric', str(newcost)]
            ...
            call_list = ['ip', 'route', 'replace', f"{ipaddr}/{slash(aton(netmask))}", 'via', update_sender, 'metric', str(cost)]
